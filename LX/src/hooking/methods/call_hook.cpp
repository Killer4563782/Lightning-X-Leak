...
namespace
{
	call_hook_memory g_call_hook_memory;
}
...
{
	m_memory = VirtualAlloc((void*)((uintptr_t)GetModuleHandle(nullptr) + 0x20000000), 1024, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	m_offset = 0;
}
	...
	m_hook(hook)
{
	...
	*(int32_t*)&m_patched_bytes[1] = (int32_t)((uint64_t)seq - (uint64_t)location - 5);
	...
	m_original_function = mem(location).add(1).rip().as<void*>();
}
