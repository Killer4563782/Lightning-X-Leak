...
inline bool is_player_model_valid(const player_ptr& player, uint32_t model)
{
	if (!player)
		return true;
	...
	if (!protections::is_valid_player_model(model))
		return false;
...
inline CObject* get_entity_attached_to(CObject* entity)
{
	...
	int64_t component = *reinterpret_cast<int64_t*>(reinterpret_cast<int64_t>(entity->gap50) + 0x48);

	if (!component)
		return nullptr;

	int unk_count = *reinterpret_cast<int*>(component + 0x5C) & 0xF;

	if (unk_count < 2)
		return nullptr;

	return *reinterpret_cast<CObject**>(component);
}
...
inline bool is_attachment_infinite(rage::netObject* object, uint16_t attached_to_net_id, int from_bone, int to_bone)
{
	...
	auto target = g_pointers->g_get_net_object(*g_pointers->g_network_object_mgr, attached_to_net_id, false);
...
inline bool is_in_vehicle(CPed* ped, CVehicle* vehicle)
{
	...
	return std::ranges::find_if(vehicle->m_passengers, [&](CPed* passenger)
	{
		return passenger == ped;
	});
}
...
inline bool is_local_player_an_occupant(CVehicleProximityMigrationDataNode* node)
{
	for (int i = 0; i < 16; i++)
	{
		if (self::local_ped && node->m_has_occupants[i] && node->m_occupants[i] == self::local_ped->m_net_object->m_object_id)
			return true;
	}
...
{
	auto x_pos = static_cast<uint32_t>((x + 149 + 8192) / 75.f);
	auto y_pos = static_cast<uint32_t>((y + 149 + 8192) / 75.f);

	return x_pos >= UCHAR_MAX || y_pos >= UCHAR_MAX;
}
...
inline std::string get_task_type_string(int type)
{
	...
	for (auto& value : tables::task_type_names)
	{
		if (value.first == type)
		{
			buffer += value.second;
...
{
	auto net_obj = g_pointers->g_get_net_object(*g_pointers->g_network_object_mgr, net_id, false);
...
{
	if (static_cast<int>(static_cast<int64_t>(*reinterpret_cast<int16_t*>(&interior_index))) >= static_cast<int>((*g_pointers->g_interior_proxy_pool)->m_size)) // this is the bug, should have been an unsigned compare instead
		return false;

	// some more checks that aren't broken

	auto a1 = reinterpret_cast<uint16_t*>(&interior_index);
...
{
	if (static_cast<uint32_t>(static_cast<int64_t>(*reinterpret_cast<int16_t*>(&interior_index))) >= (*g_pointers->g_interior_proxy_pool)->m_size)
		return false;

	return true;
}

#define LOG_FIELD_H(type, field) LOG_DEBUG("\t " #field ": 0x{:X}", (((type*)(node))->field));
#define LOG_FIELD(type, field) LOG_DEBUG("\t " #field ": {}", ((((type*)(node))->field)));
#define LOG_FIELD_C(type, field) LOG_DEBUG("\t " #field ": {}", (int)((((type*)(node))->field)));
#define LOG_FIELD_B(type, field) LOG_DEBUG("\t " #field ": {}", ((((type*)(node))->field) ? "YES" : "NO"));
#define LOG_FIELD_V3(type, field) LOG_DEBUG("\t " #field ": X: {} Y: {} Z: {}", ((((type*)(node))->field)).x, ((((type*)(node))->field)).y, ((((type*)(node))->field)).z);
#define LOG_FIELD_V4(type, field) LOG_DEBUG("\t " #field ": X: {} Y: {} Z: {} W: {}", ((((type*)(node))->field)).x, ((((type*)(node))->field)).y, ((((type*)(node))->field)).z, ((((type*)(node))->field)).w);
#define LOG_FIELD_APPLY(type, field, func) LOG_DEBUG("\t "#field ": {}", func((((type*)(node))->field)));
...
void log_node(const sync_trees::sync_node_id& node_id, const player_ptr& sender, CProjectBaseSyncDataNode* node, rage::netObject* object)
{
#ifdef _DEBUG
	if (!commands::settings::debug::log_nodes)
		return;

	if (object)
		LOG_DEBUG("{}: {}, {}", sender->get_name(), node_id.name, object->m_object_id);
	else
		LOG_DEBUG("{}: {}", sender->get_name(), node_id.name);

	switch (node_id)
	{
	case sync_trees::sync_node_id("CVehicleCreationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPedCreationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CObjectCreationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CTrainGameStateDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CDynamicEntityGameStateDataNode"):
		...
		for (int i = 0; i < reinterpret_cast<CDynamicEntityGameStateDataNode*>(node)->m_decor_count; i++)
		{
		...
		break;
	case sync_trees::sync_node_id("CAutomobileCreationDataNode"):
		LOG_FIELD_B(CAutomobileCreationDataNode, m_all_doors_closed);
		break;
	case sync_trees::sync_node_id("CDoorCreationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CDoorMovementDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CDoorScriptGameStateDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CDoorScriptInfoDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CEntityScriptGameStateDataNode"):
	...
	case sync_trees::sync_node_id("CPedAttachDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPedComponentReservationDataNode"):
		LOG_FIELD(CPedComponentReservationDataNode, m_num_peds_using_component);
		break;
	case sync_trees::sync_node_id("CPedGameStateDataNode"):
		...
		if (reinterpret_cast<CPedGameStateDataNode*>(node)->m_num_weapon_components <= 12)
		{
			for (int i = 0; i < reinterpret_cast<CPedGameStateDataNode*>(node)->m_num_weapon_components; i++)
			{
		...
		if (reinterpret_cast<CPedGameStateDataNode*>(node)->m_num_equiped_gadgets <= 3)
		{
			for (int i = 0; i < reinterpret_cast<CPedGameStateDataNode*>(node)->m_num_equiped_gadgets; i++)
			{
		...
		break;
	case sync_trees::sync_node_id("CPedHealthDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPedMovementDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPedOrientationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPedScriptCreationDataNode"):
		LOG_FIELD_B(CPedScriptCreationDataNode, m_stay_in_car_when_jacked);
		break;
	case sync_trees::sync_node_id("CPedTaskSpecificDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalAngVelocityDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalAttachDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalHealthDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalMigrationDataNode"):
		LOG_FIELD_B(CPhysicalMigrationDataNode, m_unk);
		//
		break;
	case sync_trees::sync_node_id("CPhysicalScriptMigrationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalVelocityDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPickupCreationDataNode"):
		...
		if (reinterpret_cast<CPickupCreationDataNode*>(node)->m_num_weapon_components <= 12)
		{
			for (int i = 0; i < reinterpret_cast<CPickupCreationDataNode*>(node)->m_num_weapon_components; i++)
			{
		...
		break;
	case sync_trees::sync_node_id("CPickupPlacementCreationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPlayerCameraDataNode"):
	{
		...
		auto unk_pos = reinterpret_cast<rage::fvector3*>(reinterpret_cast<int64_t>(node) + 0xF0);
		LOG_DEBUG("\t unk_pos: X: {} Y: {} Z: {}", unk_pos->x, unk_pos->y, unk_pos->z);
		break;
	}
	case sync_trees::sync_node_id("CPlayerCreationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPlayerExtendedGameStateNode"):
		//LOG_FIELD(CPlayerExtendedGameStateNode, waypoint_x);
		//LOG_FIELD(CPlayerExtendedGameStateNode, waypoint_y);

		break;
	case sync_trees::sync_node_id("CPlayerGameStateDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPlayerSectorPosNode"):
		...
		break;
	case sync_trees::sync_node_id("CGlobalFlagsDataNode"):
		LOG_FIELD(CGlobalFlagsDataNode, m_global_flags);
		LOG_FIELD(CGlobalFlagsDataNode, m_ownership_token);
		break;
	case sync_trees::sync_node_id("CMigrationDataNode"):
	...
	case sync_trees::sync_node_id("CSectorPositionDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CVehicleGadgetDataNode"):
		...
		if (reinterpret_cast<CVehicleGadgetDataNode*>(node)->m_gadget_count <= 2)
		{
			for (int i = 0; i < reinterpret_cast<CVehicleGadgetDataNode*>(node)->m_gadget_count; i++)
			{
				LOG_FIELD(CVehicleGadgetDataNode, m_gadget_data[i].m_gadget_type);
			}
		}
		break;
	case sync_trees::sync_node_id("CVehicleProximityMigrationDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPlayerGamerDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalGameStateDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPhysicalScriptGameStateDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CPedTaskTreeDataNode"):
		LOG_FIELD(CPedTaskTreeDataNode, m_task_bitset);
		for (int i = 0; i < 8; i++)
		{
			if (reinterpret_cast<CPedTaskTreeDataNode*>(node)->m_task_bitset & (1 << i))
			{
		...
		break;
	case sync_trees::sync_node_id("CPedTaskSequenceDataNode"):
		...
		for (int i = 0; i < reinterpret_cast<CPedTaskSequenceDataNode*>(node)->m_num_tasks_in_sequence; i++)
		{
	...
	case sync_trees::sync_node_id("CPlayerWantedAndLOSDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CVehicleControlDataNode"):
		...
		break;
	case sync_trees::sync_node_id("CVehicleComponentReservationDataNode"):
		...
		for (int i = 0; i < reinterpret_cast<CVehicleComponentReservationDataNode*>(node)->m_num_peds_using_component; i++)
			LOG_FIELD_NI(CVehicleComponentReservationDataNode, m_peds_using_component[i]);
		break;
	case sync_trees::sync_node_id("CPlayerAmbientModelStreamingNode"):
		...
		break;
	case sync_trees::sync_node_id("CEntityScriptInfoDataNode"):
		LOG_FIELD_B(CEntityScriptInfoDataNode, m_has_script_info);
		//LOG_FIELD_SI(CEntityScriptInfoDataNode, m_script_info);
		break;
	case sync_trees::sync_node_id("CPlayerAppearanceDataNode"):
...
bool get_player_sector_pos(rage::netSyncNodeBase* node, float& x, float& y, rage::netObject* object)
{
	...
	{
		const auto& node_id = sync_trees::sync_node_finder::find(static_cast<eNetObjType>(object->m_object_type), reinterpret_cast<uintptr_t>(node));

		if ((reinterpret_cast<CProjectBaseSyncDataNode*>(node)->flags & 1) == 0)
			return false;

		if (node_id == sync_trees::sync_node_id("CPlayerSectorPosNode"))
		{
			auto player_sector_pos_node = reinterpret_cast<CPlayerSectorPosNode*>(node);
...
bool check_node(rage::netSyncNodeBase* node, CNetGamePlayer* sender, rage::netObject* object)
{
	...
	else if (node->IsDataNode())
	{
		...
		player_ptr sender_plyr;
		if (sender)
			sender_plyr = g_player_manager->get_by_id(sender->m_player_id);

		const auto& node_id = sync_trees::sync_node_finder::find(static_cast<eNetObjType>(object->m_object_type), addr);
...
#ifdef _DEBUG
		if (sender_plyr)
			log_node(node_id, sender_plyr, reinterpret_cast<CProjectBaseSyncDataNode*>(node), object);
#endif
		...
		case sync_trees::sync_node_id("CVehicleCreationDataNode"):
		{
			const auto creation_node = reinterpret_cast<CVehicleCreationDataNode*>(node);
			if (protections::is_crash_vehicle(creation_node->m_model))
			{
				// TODO: remove some false models from is_crash_vehicle
				LOG_DEBUG("Crash A7 Model: 0x{:X}", creation_node->m_model);
				if (crash.process(sender, "Crash A7")) // invalid vehicle model
					return true;
			}

			if (auto info = model_info::get_vehicle_model(creation_node->m_model))
			{
				if (auto type = vehicle_type_to_object_type(info->m_vehicle_type); type != g_syncing_object_type)
				{
					LOG_DEBUG("Vehicle Type: {}; Syncing Type: {}", static_cast<int>(type), static_cast<int>(g_syncing_object_type));
					if (crash.m_block) // vehicle model mismatch
						return true;

					// TODO: False Alerts
					//if (crash.process(sender, "Crash AX")) // vehicle model mismatch
					//	return true;
				}
			}

			if (object && self::local_ped && self::local_ped->m_vehicle && self::local_ped->m_vehicle->m_net_object &&
				self::local_ped->m_vehicle->m_net_object->m_object_id == object->m_object_id)
				return vehicle_kick.process(sender, "Vehicle Kick");
		...
		case sync_trees::sync_node_id("CDoorCreationDataNode"):
		{
			const auto creation_node = reinterpret_cast<CDoorCreationDataNode*>(node);
			if (protections::is_crash_object(creation_node->m_model))
			{
				if (crash.process(sender, "Crash A2")) // invalid door model
					return true;
			}
		...
		case sync_trees::sync_node_id("CPickupCreationDataNode"):
		{
			const auto creation_node = reinterpret_cast<CPickupCreationDataNode*>(node);
			if (creation_node->m_custom_model && protections::is_crash_object(creation_node->m_custom_model))
			{
				if (crash.process(sender, "Crash AM")) // invalid pickup model
					return true;
			}

			for (unsigned i = 0; i < creation_node->m_num_weapon_components; i++)
			{
				uint64_t buffer[20]{};
				if (!WEAPON::GET_WEAPON_COMPONENT_HUD_STATS(creation_node->m_weapon_component[i], reinterpret_cast<Any*>(buffer))) // trying to save a pointer here
				{
					if (crash.process(sender, "Crash AI")) // invalid pickup weapon component hash
						return true;
				}
			}
		...
		case sync_trees::sync_node_id("CPhysicalAttachDataNode"):
		{
			const auto attach_node = reinterpret_cast<CPhysicalAttachDataNode*>(node);
			...
			{
				if (crash.process(sender, "Crash AR")) // infinite physical attachment
					return true;
			}

			if (attach_node->m_attached && object && object->m_object_type == static_cast<int16_t>(eNetObjType::NET_OBJ_TYPE_TRAILER))
			{
				if (auto net_obj = g_pointers->g_get_net_object(*g_pointers->g_network_object_mgr, attach_node->m_attached_to, false))
				{
					if (auto entity = net_obj->GetGameObject())
					{
						if (entity->m_entity_type != 3)
						{
							if (crash.process(sender, "Crash A9")) // invalid attachment
								return true;
						}
					}
				}
		...
		case sync_trees::sync_node_id("CPedCreationDataNode"):
		{
			const auto creation_node = reinterpret_cast<CPedCreationDataNode*>(node);
			if (protections::is_crash_ped(creation_node->m_model))
			{
				if (crash.process(sender, "Crash A3")) // invalid ped model
					return true;
			}
			if (creation_node->m_has_prop && protections::is_crash_object(creation_node->m_prop_model))
			{
				if (crash.process(sender, "Crash A5")) // invalid ped prop model
					return true;
			}
			//if (creation_node->m_is_respawn_object_id)
			break;
		}
		case sync_trees::sync_node_id("CPedAttachDataNode"):
		{
			const auto attach_node = reinterpret_cast<CPedAttachDataNode*>(node);
			if (attach_node->m_attached
				&& (object->m_object_id == attach_node->m_attached_to
					|| is_attachment_infinite(object,
						attach_node->m_attached_to,
						...
						attach_node->m_attachment_bone)))
			{
				if (auto game_object = reinterpret_cast<CPed*>(object->GetGameObject()))
					if (!game_object->m_player_info)
						if (crash.process(sender, "Crash AZ")) // infinite ped attachment - parachute false positives
							return true;
			}
		...
		case sync_trees::sync_node_id("CObjectCreationDataNode"):
		{
			const auto creation_node = reinterpret_cast<CObjectCreationDataNode*>(node);

			if (protections::is_cage_object(creation_node->m_model))
			{
				if (cage.process(sender, "Cage"))
					return true;
			}

			if (protections::is_crash_object(creation_node->m_model))
			{
				// TODO: remove some false models from is_crash_object
				LOG_DEBUG("Crash A6 Model: 0x{:X}", creation_node->m_model);
				if (crash.process(sender, "Crash A6")) // invalid object model
					return true;
			}

			if (creation_node->m_created_by == static_cast<uint32_t>(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(sender_plyr->id())) && creation_node->m_is_broken)
			{
				if (crash.process(sender, "Crash AU")) // broken object
					return true;
			}
			break;
		}
		case sync_trees::sync_node_id("CPlayerAppearanceDataNode"):
		{
			const auto player_appearance_node = reinterpret_cast<CPlayerAppearanceDataNode*>(node);
			if (protections::is_crash_ped(player_appearance_node->m_model_hash) || !is_player_model_valid(sender_plyr, player_appearance_node->m_model_hash))
			{
				if (crash.process(sender, "Crash AT")) // invalid player model (appearance node)
					return true;
			}
		...
		case sync_trees::sync_node_id("CPlayerCreationDataNode"):
		{
			const auto player_creation_node = reinterpret_cast<CPlayerCreationDataNode*>(node);
			if (protections::is_crash_ped(player_creation_node->m_model) || !is_player_model_valid(sender_plyr, player_creation_node->m_model))
			{
				LOG_DEBUG("Crash AD Model: 0x{:X}", player_creation_node->m_model);
				if (crash.process(sender, "Crash AD")) // invalid player model (creation node)
					return true;
			}
			break;
		}
		case sync_trees::sync_node_id("CSectorDataNode"):
		{
			if (static_cast<eNetObjType>(object->m_object_type) == eNetObjType::NET_OBJ_TYPE_PLAYER)
			{
				...
				const auto sector_node = reinterpret_cast<CSectorDataNode*>(node);
				float pos_x = (sector_node->m_pos_x - 512.0f) * 54.0f;
				float pos_y = (sector_node->m_pos_y - 512.0f) * 54.0f;
				if (is_invalid_override_pos(pos_x + player_sector_pos_x, pos_y + player_sector_pos_y))
				{
#ifdef _DEBUG
					std::stringstream crash_reason;
					crash_reason << "invalid sector position (sector node)" << " X: " << pos_x << " Y: " << pos_y << " player_sector_pos_x: " << player_sector_pos_x << " player_sector_pos_y: " << player_sector_pos_y;
					LOG_DEBUG(crash_reason.str());
#endif
					if (crash.process(sender, "Crash AB")) // invalid sector position (sector node)
						return true;
				}
			}
			break;
		...
		case sync_trees::sync_node_id("CPlayerGameStateDataNode"):
		{
			const auto game_state_node = reinterpret_cast<CPlayerGameStateDataNode*>(node);
			...
			{
				if ((*g_pointers->g_network)->m_game_session_ptr->is_host())
					if (crash.process(sender, "Crash AF")) // invalid sector position (player game state node)
						return true;
			}
			...
			if (sender_plyr)
			{
				...
				{
					notify(Warning, "{} Triggered A Detection: Super Jump");
				}

				if (!game_state_node->m_is_max_armor_and_health_default && game_state_node->m_max_health == 0.0f && game_state_node->m_player_state == 1)
				{
				...
				if (game_state_node->m_is_spectating)
				{
					...
					auto net_obj = g_pointers->g_get_net_object(*g_pointers->g_network_object_mgr, game_state_node->m_spectating_net_id, false);
					...
					auto player_info = reinterpret_cast<CPed*>(entity)->m_player_info;
					...
					if (self::local_ped && reinterpret_cast<CPed*>(entity) == self::local_ped)
					{
						target = g_player_manager->get_self();
					}
					else
					{
						for (auto p : g_player_manager->players())
					...
					// Dont really need this. Maybe self for notifying
		...
		case sync_trees::sync_node_id("CTrainGameStateDataNode"):
		{
			const auto train_node = reinterpret_cast<CTrainGameStateDataNode*>(node);
			int track_id = train_node->m_track_id;
			if (track_id != -1 && (track_id < 0 || track_id >= 12))
			{
				LOG_DEBUG("Out of bouns train track index: {}", static_cast<int>(train_node->m_track_id));
				if (crash.process(sender, "Crash AQ")) // out of bounds train track index
					return true;
			}
			...
			{
				if (static_cast<uint32_t>(train_config_index) >= g_pointers->g_train_config_array->size())
				{
					if (crash.process(sender, "Crash AN")) // out of bounds train config index
						return true;
				}

				if (carriage_config_index != -1)
				{
					if (static_cast<uint32_t>(carriage_config_index) >= (*g_pointers->g_train_config_array)[train_config_index].m_carraige_configs.size())
					{
						if (crash.process(sender, "Crash AJ")) // out of bounds carriage config index
							return true;
					}
				}
			}
		...
		case sync_trees::sync_node_id("CVehicleProximityMigrationDataNode"):
		{
			if (object && self::local_ped && self::local_ped->m_net_object)
			{
				const auto migration_node = reinterpret_cast<CVehicleProximityMigrationDataNode*>(node);

				if (!self::local_ped->m_vehicle || !self::local_ped->m_vehicle->m_net_object
					|| self::local_ped->m_vehicle->m_net_object->m_object_id != object->m_object_id
					|| !is_in_vehicle(self::local_ped, self::local_ped->m_vehicle))
				{
		...
		case sync_trees::sync_node_id("CPlayerGamerDataNode"):
		{
			const auto gamer_node = reinterpret_cast<CPlayerGamerDataNode*>(node);

			if (sender_plyr)
			{
				if (gamer_node->m_clan_data.m_clan_id == 123456 && gamer_node->m_clan_data.m_clan_id_2 == 123456)
				{
					// TODO: Add infractions
		...
		case sync_trees::sync_node_id("CPedGameStateDataNode"):
		{
			const auto game_state_node = reinterpret_cast<CPedGameStateDataNode*>(node);
			if (game_state_node->m_weapon_hash == "WEAPON_STRICKLER"_joaat)
			{
				if (crash.process(sender, "Crash AK")) // invalid weapon hash
					return true;
			}
			if (game_state_node->m_on_mount)
			{
				if (crash.process(sender, "Crash AE")) // mount flag
					return true;
			}
			for (unsigned i = 0; i < game_state_node->m_num_equiped_gadgets; i++)
			{
				if (game_state_node->m_gadget_hash[i] != "gadget_parachute"_joaat && game_state_node->m_gadget_hash[i] != "gadget_nightvision"_joaat)
				{
					if (crash.process(sender, "Crash AG")) // invalid gadget
						return true;
				}
			}
			break;
		...
		case sync_trees::sync_node_id("CVehicleControlDataNode"):
		{
			const auto control_node = reinterpret_cast<CVehicleControlDataNode*>(node);
			if (control_node->m_is_submarine_car)
			{
				if (auto vehicle = reinterpret_cast<CVehicle*>(object->GetGameObject()))
				{
					if (auto model_info = reinterpret_cast<CVehicleModelInfo*>(vehicle->m_model_info))
					{
						if (model_info->m_vehicle_type != eVehicleType::VEHICLE_TYPE_SUBMARINECAR)
						{
							if (crash.process(sender, "Crash AH")) // submarine car (sync)
								return true;
						}
					}
				}
				else if (veh_creation_model != std::nullopt)
				{
					...
					if (auto model_info = model_info::get_vehicle_model(veh_creation_model.value()))
					{
						if (model_info->m_vehicle_type != eVehicleType::VEHICLE_TYPE_SUBMARINECAR)
						{
							if (crash.process(sender, "Crash AC")) // submarine car (creation)
								return true;
						}
					}
				}
		...
		case sync_trees::sync_node_id("CPlayerCameraDataNode"):
		{
			const auto camera_node = reinterpret_cast<CPlayerCameraDataNode*>(node);
			if (is_invalid_override_pos(camera_node->m_free_cam_pos_x, camera_node->m_free_cam_pos_y))
			{
				if ((*g_pointers->g_network)->m_game_session_ptr->is_host())
					if (crash.process(sender, "Crash AP")) // invalid sector position (camera data node)
						return true;
			}
		...
		case sync_trees::sync_node_id("CVehicleGadgetDataNode"):
		{
			const auto gadget_node = reinterpret_cast<CVehicleGadgetDataNode*>(node);
			for (unsigned i = 0; i < gadget_node->m_gadget_count; i++)
			{
				if (gadget_node->m_gadget_data[i].m_gadget_type > 7u)
				{
					if (crash.process(sender, "Crash A1")) // out of bounds gadget type
						return true;
				}
			}
			break;
		...
		case sync_trees::sync_node_id("CPedTaskTreeDataNode"):
		{
			const auto task_node = reinterpret_cast<CPedTaskTreeDataNode*>(node);

			for (int i = 0; i < 8; i++)
			{
				if (task_node->m_task_bitset & (1 << i))
				{
					if (is_crash_ped_task(static_cast<eTaskTypeIndex>(task_node->m_tasks[i].m_task_type)))
					{
						LOG_DEBUG("Invalid Ped Task: {}", task_node->m_tasks[i].m_task_type);

						if (crash.process(sender, "Crash AW")) // invalid ped task
							return true;
					}
				}
			}
		...
		case sync_trees::sync_node_id("CVehicleTaskDataNode"):
		{
			const auto task_node = reinterpret_cast<CVehicleTaskDataNode*>(node);
			if (is_crash_vehicle_task(static_cast<eTaskTypeIndex>(task_node->m_task_type)))
			{
				LOG_DEBUG("Syncing Object: {} {}", static_cast<int>(g_syncing_object_type), get_task_type_string(task_node->m_task_type));
				if (crash.m_block) // invalid vehicle task
					return true;

				// TODO: False Alerts
				//if (crash.process(sender, "Crash AV")) // invalid vehicle task
				//{
				//	LOG_DEBUG("Syncing Object: {} {}", (int)g_syncing_object_type, get_task_type_string(task_node->m_task_type));
				//	return true;
				//}
			}
		...
		case sync_trees::sync_node_id("CEntityScriptInfoDataNode"):
		{
			const auto script_node = reinterpret_cast<CEntityScriptInfoDataNode*>(node);
			if (script_node->m_has_script_info)
			{
				if (script_node->m_script_info.m_local_handle == 0)
				{
					if (crash.process(sender, "Crash A8")) // invalid script info
						return true;
				}
			}
			break;
		...
		case sync_trees::sync_node_id("CDynamicEntityGameStateDataNode"):
		{
			const auto game_state_node = reinterpret_cast<CDynamicEntityGameStateDataNode*>(node);

			if (is_valid_interior_game(game_state_node->m_interior_index) && !is_valid_interior_fixed(game_state_node->m_interior_index)) // will crash
			{
				if (crash.process(sender, "Crash A4")) // invalid interior
					return true;
			}
...
{
	static bool init = ([] { sync_trees::sync_node_finder::init(); }(), true);
	...
	return g_hooking->get_original<can_apply_data>()(tree, object);
}