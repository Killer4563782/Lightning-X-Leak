...
namespace image_loader
{
	...
	std::map<int, img_data> load_gif(const fs::path& path)
	{
		...
		const auto data = stbi_load_gif_from_memory(reinterpret_cast<stbi_uc*>(buffer.data()), static_cast<int>(size), &delays, &width, &height, &frame_count, &comp, 0);
		if (!data)
		{
			return frames;
		}
	...
	ID3D11ShaderResourceView* create_resource_view(unsigned char* img_data, const vec2 img_size)
	{
		...
		g_renderer->m_d3d_device->CreateTexture2D(&desc, &sub_resource, &p_texture);
		D3D11_SHADER_RESOURCE_VIEW_DESC srv_desc = {};
		srv_desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
		srv_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
		srv_desc.Texture2D.MipLevels = desc.MipLevels;
		ID3D11ShaderResourceView* tmp_res = nullptr;
		g_renderer->m_d3d_device->CreateShaderResourceView(p_texture, &srv_desc, &tmp_res);
		p_texture->Release();
		return tmp_res;
	}
	...
	void header_handler()
	{
		if (g_open && has_header_loaded)
		{
			flip_through_frames(m_header[m_header_frame].frame_delay);
			drawing::image(m_header[m_header_frame].res_view, { g_pos.x, g_pos.y }, { g_width, g_header.m_size }, g_options.m_text_color);
		}
	}
}
