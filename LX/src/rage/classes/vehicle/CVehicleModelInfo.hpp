...
public:
    ...
    uint32_t get_class()
	{
		return static_cast<int>(m_vehicle_class) & 0x1F;
	}

	bool is_class(eVehicleClass vehicleClass)
	{
		return get_class() == static_cast<uint32_t>(vehicleClass);
	}

	bool get_vehicle_model_flag(const CVehicleModelInfoFlags flag)
	{
	...
	void set_vehicle_model_flag(const CVehicleModelInfoFlags flag, bool toggle)
	{
		const auto index = static_cast<uint16_t>(flag);
