...
namespace sync_trees
{
	...
	public:
		...
		static void init()
		{
			for (int i = static_cast<int>(eNetObjType::NET_OBJ_TYPE_AUTOMOBILE); i <= static_cast<int>(eNetObjType::NET_OBJ_TYPE_TRAIN); i++)
			{
				if (i == static_cast<int>(eNetObjType::NET_OBJ_TYPE_TRAILER))
					continue;

				rage::netSyncTree* tree = g_pointers->g_get_sync_tree_for_type(*g_pointers->g_network_object_mgr, i);

				if (tree->m_child_node_count != finder.sync_trees_node_array_index_to_node_id[i].size())
				{
					LOG(critical, "Cache Nodes failed {} {} {}", i, tree->m_child_node_count, finder.sync_trees_node_array_index_to_node_id[i].size());

					throw std::runtime_error("Failed to Cache Nodes");
				}

				for (unsigned j = 0; j < tree->m_child_node_count; j++)
				{
					const uintptr_t addr = reinterpret_cast<uintptr_t>(tree->m_child_nodes[j]);
