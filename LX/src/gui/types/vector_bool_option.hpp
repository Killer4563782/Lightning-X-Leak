...
namespace ui
{
	class vector_bool_option final : public option
	{
	public:
		bool* m_toggle;
		int* m_index;
		...
		explicit vector_bool_option(std::string name, std::string description, bool* toggle, int* index, std::vector<std::string>* vector, std::function<void()> function = nullptr) : option(std::move(name), std::move(description), OptionType::VectorBool),
			m_toggle(toggle),
			m_index(index),
			m_vector(vector),
		...
		explicit vector_bool_option(std::string name, std::string description, commands::looped_vector_command* cmd) : option(std::move(name), std::move(description), OptionType::VectorBool),
			m_toggle(&cmd->m_toggle),
			m_index(&cmd->m_index),
			m_vector(&cmd->m_vector)
		{}

		explicit vector_bool_option(std::string name, commands::looped_vector_command* cmd) : vector_bool_option(std::move(name), cmd->m_description, cmd)
		{}

		explicit vector_bool_option(commands::looped_vector_command* cmd) : vector_bool_option(cmd->m_name, cmd->m_description, cmd)
		{}
