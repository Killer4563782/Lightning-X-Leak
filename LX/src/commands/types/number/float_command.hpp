...
{
	class float_command;
	inline std::vector<float_command*> g_float_commands;
	...
	public:
		...
		float_command(const std::string& name, const std::string& command, const std::string& description, bool requires_fiber_pool,
				float min, float max, float step = 1.f, float value = 0.f) :
			...
			m_step(step)
		{
			g_float_commands.push_back(this);
		}
