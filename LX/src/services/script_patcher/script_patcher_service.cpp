...
script_data* script_patcher_service::get_data_for_script(rage::joaat_t script) const
{
...
{
	return std::ranges::any_of(m_script_patches, [script](const script_patch& p) { return p.get_script() == script; });
}
...
void script_patcher_service::update()
{
	for (auto& p : m_script_patches)
	{
		if (auto data = get_data_for_script(p.get_script()))
		{
			p.update(data);
		}
	}
...
void script_patcher_service::disable_all()
{
	for (auto& p : m_script_patches)
	{
		if (auto data = get_data_for_script(p.get_script()))
