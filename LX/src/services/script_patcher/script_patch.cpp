...
script_patch::script_patch(rage::joaat_t script, std::string name, memory::pattern pattern, int32_t offset, std::vector<uint8_t> patch, bool* enable_bool) :
	m_script(script),
...
std::optional<uint32_t> script_patch::get_code_location_by_pattern(script_data* data, const memory::pattern& pattern)
{
...
void script_patch::enable(script_data* data) const
{
...
void script_patch::disable(script_data* data) const
{
	std::lock_guard lock(g_patches_mutex);
...
void script_patch::update(script_data* data)
{
	if (m_ip == 0)
	{
		...
		if (!result.has_value())
		{
			LOG(error, "Failed to find pattern: {}", m_name);
			return;
		}
